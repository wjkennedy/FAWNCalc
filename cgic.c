...
static cgiParseResultType cgiParseDatabaseFormInput(int paramset);
...
/* following is by H. Dietz */
char *
cgiRepeatQ(char *addon)
{
	static char repeatbuf[64 * 1024];
...
cgiOutem(void)
{
	...
	fprintf(cgiOut, "<P>ServerSoftware = %s\n", cgiServerSoftware);
	fprintf(cgiOut, "<P>ServerName = %s\n", cgiServerName);
	fprintf(cgiOut, "<P>GatewayInterface = %s\n", cgiGatewayInterface);
	fprintf(cgiOut, "<P>ServerProtocol = %s\n", cgiServerProtocol);
	fprintf(cgiOut, "<P>ServerPort = %s\n", cgiServerPort);
	fprintf(cgiOut, "<P>RequestMethod = %s\n", cgiRequestMethod);
	fprintf(cgiOut, "<P>PathInfo = %s\n", cgiPathInfo);
	fprintf(cgiOut, "<P>PathTranslated = %s\n", cgiPathTranslated);
	fprintf(cgiOut, "<P>ScriptName = %s\n", cgiScriptName);
	fprintf(cgiOut, "<P>QueryString = %s\n", cgiQueryString);
	fprintf(cgiOut, "<P>RemoteHost = %s\n", cgiRemoteHost);
	fprintf(cgiOut, "<P>RemoteAddr = %s\n", cgiRemoteAddr);
	fprintf(cgiOut, "<P>AuthType = %s\n", cgiAuthType);
	fprintf(cgiOut, "<P>RemoteUser = %s\n", cgiRemoteUser);
	fprintf(cgiOut, "<P>RemoteIdent = %s\n", cgiRemoteIdent);
	fprintf(cgiOut, "<P>ContentType = %s\n", cgiContentType);
	fprintf(cgiOut, "<P>Accept = %s\n", cgiAccept);
	fprintf(cgiOut, "<P>UserAgent = %s\n", cgiUserAgent);
	fprintf(cgiOut, "<P>Referrer = %s\n", cgiReferrer);
	fprintf(cgiOut, "<P>ContentLength = %d\n", cgiContentLength);
...
/* following is by H. Dietz */
int
cgiMailEnvironment(char *from, char *to, char *path, char *subject)
{
	char mailfile[64];
	FILE *out;
	cgiFormEntry *e;
	...
	sprintf(mailfile, "%s_XXXXXX", path);
	if (mkstemp(mailfile) == 0) return(1);
	if ((out = fopen(mailfile, "w")) == 0) return(2);
	chmod(mailfile, (S_IRUSR | S_IWUSR |
			 S_IRGRP | S_IWGRP |
			 S_IROTH | S_IWOTH));
	...
	fprintf(out, "Subject: Assignment %s\n", subject);
	fprintf(out, "\n\n");
...
*/
	...
	fprintf(out, "\nAssignmentLogFile = %s\n", mailfile);
	fprintf(out, "ServerSoftware = %s\n", cgiServerSoftware);
	fprintf(out, "ServerName = %s\n", cgiServerName);
	fprintf(out, "GatewayInterface = %s\n", cgiGatewayInterface);
	fprintf(out, "ServerProtocol = %s\n", cgiServerProtocol);
	fprintf(out, "ServerPort = %s\n", cgiServerPort);
	fprintf(out, "RequestMethod = %s\n", cgiRequestMethod);
	fprintf(out, "PathInfo = %s\n", cgiPathInfo);
	fprintf(out, "PathTranslated = %s\n", cgiPathTranslated);
	fprintf(out, "ScriptName = %s\n", cgiScriptName);
	fprintf(out, "QueryString = %s\n", cgiQueryString);
	fprintf(out, "RemoteHost = %s\n", cgiRemoteHost);
	fprintf(out, "RemoteAddr = %s\n", cgiRemoteAddr);
	fprintf(out, "AuthType = %s\n", cgiAuthType);
	fprintf(out, "RemoteUser = %s\n", cgiRemoteUser);
	fprintf(out, "RemoteIdent = %s\n", cgiRemoteIdent);
	fprintf(out, "ContentType = %s\n", cgiContentType);
	fprintf(out, "Accept = %s\n", cgiAccept);
	fprintf(out, "UserAgent = %s\n", cgiUserAgent);
	fprintf(out, "Referrer = %s\n", cgiReferrer);
	fprintf(out, "ContentLength = %d\n", cgiContentLength);
	...
	fprintf(cgiOut, "Assignment %s logged and emailed.\n", subject);
	fprintf(cgiOut, "<P>\n");
...
/* following is by H. Dietz */
int
cgiMailEnvBoth(char *from, char *to, char *path, char *subject)
{
	char mailfile[64];
	FILE *out;
	cgiFormEntry *e;
...
cgiFormResultType cgiLoadDB(int paramset)
{
	/* need to load the database from a file */
	if (cgiParseDatabaseFormInput(paramset) != cgiParseSuccess) {
		cgiFreeResources();
		return -1;
	}
...
/* HACK ALERT: Read in the database from a file and append it
 * to the data that came from standard input, thus making the
 * CGI think it is getting form input for the database.
 */
static cgiParseResultType cgiParseDatabaseFormInput(int paramset) {
	char *input;
	cgiParseResultType result;
	...
	snprintf(dbfn, sizeof(dbfn), "db%d.cgtxt", paramset);

	if ((result = stat(dbfn, &statBuf)) == -1) {
		return cgiParseIO;
	}
	input = (char *) malloc(statBuf.st_size);
	if (!input) {
		return cgiParseMemory;	
	}
	if ((db = fopen(dbfn, "r")) == NULL) {
		return cgiParseIO;
	}
	if (flock(fileno(db), LOCK_SH) < 0) {
	  err_exit("Cannot lock database for uid %d\n", paramset);
	}
	if (fread(input, 1, statBuf.st_size, db) != statBuf.st_size) {
		return cgiParseIO;
	}
	if (flock(fileno(db), LOCK_UN) < 0) {
	  err_exit("Cannot unlock database for uid %d\n", paramset);
	}
	fclose(db);
	result = cgiParseFormInput(input, statBuf.st_size);
	free(input);
	return result;
}
